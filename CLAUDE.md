...
- **AI生成コンテンツの明記**: 冒頭に `Callout` コンポーネントで「Claude Deep Researchによるアウトプット（YYYY/MM）」と明記
- **見出しへのリンク禁止**: fumadocsのh3見出しは自動的にアンカーリンクでラップされるため、見出しテキスト内にリンクを入れると「aタグの中にaタグ」になりhydration errorが発生
- **リンク配置**: 各ツール/サービスの説明の末尾に「**Links:** [URL]」という形式で配置

#### 一般的な文体
- **簡潔さを優先**: 必要最小限の情報のみを記載
