...
- **AI文体レビュー**: AIが本文を新規生成または大幅に編集した場合、保存前に[`textlint-rule-preset-ai-writing`](https://github.com/textlint-ja/textlint-rule-preset-ai-writing)の観点でセルフレビューする。太字ラベル付きリスト、絵文字による装飾、誇張・断定表現、機械的な強調、述語の直後にコロンを置く表現、冗長・抽象的な説明を避ける。レビュー対象はAIが追加・編集した部分だけとし、ユーザーの原文、引用、明示されたAIコメントは書き換えない
